	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="剩余数量" prop="shengyushuliang" >
					<el-input v-model.number="ruleForm.shengyushuliang" placeholder="剩余数量" clearable  :readonly="ro.shengyushuliang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="剩余数量" prop="shengyushuliang" >
					<el-input v-model="ruleForm.shengyushuliang" placeholder="剩余数量" readonly></el-input>
				</el-form-item>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `menzhenyisheng/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.menzhenyishengCrossAddOrUpdateFlag = false;
											this.parent.search();
