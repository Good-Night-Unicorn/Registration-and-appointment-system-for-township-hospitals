			...
			<view :style='{"minHeight":"100vh","width":"100%","padding":"0 0 160rpx","position":"relative","background":"#FEF9F3","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"width":"calc(100% - 0px)","padding":"20rpx ","position":"relative","background":"none","display":"flex","height":"auto"}'>
					<view :style='{"margin":"0","flex":"1","position":"relative"}' class="search-form round">
						...
						<input placeholder-style="color: #333" :style='{"border":"0px solid #27357C","padding":"12rpx 20rpx 12rpx 80rpx","boxShadow":"inset 0px 2rpx 8rpx 0px #00000040","color":"#333","borderRadius":"0","background":"#FFFFFF","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.xingming" type="text" placeholder="姓名" ></input>
					</view>
					...
					<view class="list" :style='{"width":"100%","padding":"40rpx 0 0 0","margin":"0","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"margin":"0 0 20rpx","borderRadius":"20rpx","flexWrap":"wrap","background":"#FFFFFF","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view class="listmr" :style='{"width":"calc(100% - 200rpx)","padding":"0","margin":"0","flexWrap":"wrap","display":"flex","height":"auto"}'>
								<view class="col3 f30 elip mb15" :style='{"width":"100%","padding":"10rpx 20rpx","lineHeight":"48rpx","fontSize":"32rpx","color":"#4A4A4A"}'>姓名:{{product.xingming}}</view>
								<view class="col3 f30 elip mb15" :style='{"width":"100%","padding":"10rpx 20rpx","lineHeight":"48rpx","fontSize":"32rpx","color":"#4A4A4A"}'>性别:{{product.xingbie}}</view>
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiuzhenren','修改')) || (!userid && isAuthFront('jiuzhenren','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiuzhenren','删除')) || (!userid && isAuthFront('jiuzhenren','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiuzhenren','修改')) || (!userid && isAuthFront('jiuzhenren','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiuzhenren','删除')) || (!userid && isAuthFront('jiuzhenren','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
								</view>
							</view>
				...
				<view class="screenBox" :class="screenBoxShow?'screenBoxActive':''">
					...
					<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0","alignItems":"center","flexWrap":"wrap","justifyContent":"flex-start","display":"flex"}'>
						<view :style='{"width":"18%","padding":"20rpx 0","fontSize":"28rpx","color":"#858585"}'>身份证号</view>
						<input :style='{"border":"2rpx solid #CCCCCC","width":"100%","padding":"0 20rpx 0 20rpx","borderRadius":"0","background":"#fff","height":"80rpx"}' placeholder="请输入身份证号" v-model="searchForm.shenfenzhenghao">
					</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`jiuzhenren`, params);
                } else {
                    res = await this.$api.list(`jiuzhenren`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jiuzhenren', JSON.stringify([id]));
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`jiuzhenren`, searchForm);
                } else {
                    res = await this.$api.list(`jiuzhenren`, searchForm);
                }
				// 如果是第一页数据置空
